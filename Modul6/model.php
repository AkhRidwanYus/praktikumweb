...
<body>
<?php
	function tambahmember($nama, $no, $alamat, $tgldft, $tgltrkhrbyr, $pass){
		if (!empty($_POST["addmember"])) {
			$sql = "INSERT INTO member (nama_member, nomor_member, alamat, tgl_mendaftar, tgl_terakhir_bayar, pass) VALUES (:nama, :no, :alamat, :mendaftar, :terakhir, :pass)";
			$stmt = konek()->prepare($sql);
			$result = $stmt->execute(array(':nama' => $nama, ':no' => $no, ':alamat' => $alamat, ':mendaftar' => $tgldft, ':terakhir' => $tgltrkhrbyr, ':pass' => $pass));
			if (!empty($result)) {
	...
	function updatemember($id, $nama, $no, $alamat, $tgldft, $tgltrkhrbyr, $pass){
		$pdo_statement = konek()->prepare(
			"update member set nama_member='" . $nama . "', nomor_member='" . $no . "', alamat='" . $alamat . "', tgl_mendaftar='" . $tgldft . "', tgl_terakhir_bayar='" . $tgltrkhrbyr . "', pass='" . $pass . "' where id_member=" . $id
		);
